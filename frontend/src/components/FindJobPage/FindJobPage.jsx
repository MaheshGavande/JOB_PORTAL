...
const FindJobPage = () => {
  ...
  const toggleSaveJob = async (jobId) => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/saved/job/${jobId}`, {
        method: "POST",
        headers: {
  ...
  const confirmApply = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/application/apply/${confirmToast.jobId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/application/user`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/job?${params.toString()}`,
      );
      const data = await res.json();

      if (data.success) {
        const mappedJobs = data.jobs.map((job) => {
          const logoSrc = normalizeAssetUrl(job.companyLogo);
  ...
  const renderLogo = (job) => {
    ...
    const isBroken =
      imgErrors[job.id] || !logoUrl || logoUrl === `${API_URL}`;
